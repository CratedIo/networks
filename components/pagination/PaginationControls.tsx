    ...
    <div className="flex justify-between items-center">
      ...
      <Pagination className="justify-end">
        <PaginationContent>
          ...
          {show_previous && <PaginationPrevious href={`/climate?page=${current_page - 1}`} />}
          ...
          {total_pagesArray.map((page_number:any, idx:any) => page_number + 1 <= current_page + 2 && page_number + 1 >= current_page - 2 && 
              ...
              <PaginationLink key={idx} href={`/climate?page=${page_number + 1}`} isActive={page_number + 1 == current_page ? true : false}  >{page_number + 1}</PaginationLink>

          )}
    
          {show_nextEllipse && <PaginationEllipsis />}
    
          {show_next && <PaginationNext href={`/climate?page=${current_page + 1}`} />}
